        ...
        <div className="w-full h-full">
            ...
            <div className="relative flex flex-col w-[95%] h-full overflow-scroll ml-3 mt-4">
                ...
                <div className="flex flex-row items-center justify-between w-full">
                    ...
                    <button
                        className="my-2 flex flex-row gap-2 items-center opacity-50 px-2 py-1 hover:bg-white/10 bg-white/10 hover:opacity-100 border border-white/0 hover:border-white/30  font-bold rounded-lg cursor-pointer transition-all duration-300 ease-in-out text-2xl cursor-pointer"
                        onClick={
                            () => {
                        ...
                        <div key={items.thread_id} className="flex flex-row justify-between items-center my-0.5">
                            <button
                                className={`overflow-x-hidden text-ellipsis w-full whitespace-nowrap ${items.thread_id === CurrThreadID
                                    ...
                                    : "opacity-50"
                                    } hover:opacity-100  border border-white/0 hover:border-white/30 transition-all duration-200 ease-in-out hover:bg-white/10 text-white rounded-lg px-2 py-2 cursor-pointer text-start font-semibold`}
                                onClick={() => {
                            ...
                            <button
                                onClick={(e) => {
                                    ...
                                    setthreadMenu((prev) => {
                                        if (selectedThreadMenu === items.thread_id && prev) return false;
                                        return true;
                                    })
            ...
            </div >
            {
                threadMenu && selectedThreadMenu && (
                    <div ref={threadMenuRef} className="fixed mt-1 bg-black border border-white/30 rounded-lg shadow-sm shadow-white/30 z-[101] flex flex-col p-1 w-40"
                        style={{ top: menuPosition.top, left: menuPosition.left }}
                    >
                        <button
                            onClick={async () => {
                                const { data, error } = await deleteThread(selectedThreadMenu);
                                if (error) {
                                    setalertMessage(error);
                                    setalert(true);
                                    return;
                                ...
                                setHistory((prev) => {
                                    return prev.filter((element) => selectedThreadMenu != element.thread_id)
                                })
                                setthreadMenu(false);
                            }}
